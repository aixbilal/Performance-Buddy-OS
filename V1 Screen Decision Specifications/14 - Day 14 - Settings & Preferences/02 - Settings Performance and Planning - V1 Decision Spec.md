# PBOS — Settings Performance & Planning

**Archive:** V1 Screen Decision Archive  
**Design Part:** Day 14  
**Screen / State:** Settings Performance & Planning  
**Status:** PRODUCT/UI DECIDED — V1  
**Reference:** `PBOS-Settings-Performance-Planning-v1-REFERENCE.png`
...
## Purpose
Defines baseline capacity, protected time and planning behavior used by Planner/Today.

## Locked Decisions
- Baseline capacity with flexible/preferred/hard-max semantics.
- Protected sleep/time.
- Focus configuration.
- Planning buffer (working default ~10%, configurable).
- Daily load limits.
- Preferred scheduling times.
- Priority behavior.
- Operating Modes.
- Base → Mode → Temporary → Effective view.
- Temporary overrides can expire.
- Mode preview before activation.
- Suggest → Preview → User activates.
- Missed-work default should ask the user rather than silently reschedule.
- Hard constraints differ from preferences.
- Baseline target is not a mandatory minimum.
- Optional work may remain unscheduled.
- AI cannot silently change Mode/capacity.

## Final-Audit Checks
- Protected sleep remains protected unless explicit user action changes it.
- Planner consumes effective configuration.
