...
**Design Part:** Day 02  
**Screen / State:** Focus Active  
**Status:** PRODUCT/UI DECIDED — V1  
**Reference:** `PBOS-Focus-Active-v1-PRIMARY.png`
...
## Purpose
Focus Active is the deliberate execution state for one identified piece of work.

## Locked Decisions
- Focus is not the default study mode and not merely a Pomodoro skin.
- The active session clearly shows task/topic identity, timer, progress/state, pause/resume, quick notes and finish/end-early controls.
- Timer uses the locked technical/mono treatment.
- Entering Focus is a meaningful mode transition and may use slightly more deliberate motion, while still feeling fast.
- Session lifecycle is authoritative and persistent: Start → Pause → Resume → Finish / End Early.
- Navigation/minimize must not silently kill an active session.
- Quick notes are session notes/evidence and must not become a separate long-form note system.
- A future companion micro-session area may exist, but full companion AI is not part of V1.
- Focus should be more prominent in quiz/exam preparation and weak-topic reinforcement, but the UI must not force every Action into Focus.
- The active screen protects attention: low noise, minimal unrelated cards, restrained illumination and a clear current objective.

## Evidence
Session duration, completion, quick notes and linked Action/topic become evidence. Time alone does not mean mastery.

## Final-Audit Checks
- Session survives pause/resume/navigation/minimize as designed.
- Timer persistence is correct.
- Active task identity remains clear.
- No duplicate task/session engine exists.
