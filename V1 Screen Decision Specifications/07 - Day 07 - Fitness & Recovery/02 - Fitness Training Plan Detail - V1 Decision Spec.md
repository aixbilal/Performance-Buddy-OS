# PBOS — Fitness Training Plan Detail

**Archive:** V1 Screen Decision Archive  
**Design Part:** Day 07  
**Screen / State:** Fitness Training Plan Detail  
**Status:** PRODUCT/UI DECIDED — V1  
**Reference:** `PBOS-Fitness-Training-Plan-Detail-v1-REFERENCE.png`
...
## Purpose
Training Plan Detail defines the repeatable fitness program while preserving adaptations and actual execution separately.

## Critical Lock
Always distinguish:
1. **Base Plan** — originally prescribed.
2. **Today's Prescription** — after adaptation.
3. **Actual Session** — what the user really did.

Example:
`Base 3.5 km → recovery adjusted 2.5 km easy → actual 2.7 km`

## Locked Decisions
- Adaptation must not rewrite historical/base intent.
- Plan contains sessions/exercises/targets appropriate to the goal.
- Recovery/readiness may produce a recommendation for today's prescription.
- User can accept/modify as appropriate.
- Actual performance becomes evidence after execution.
- Plan is not a Goal and is not a session history.

## Final-Audit Checks
- All three values/states are persisted independently.
- Plan edits are explicit.
- No silent “adaptive” rewriting that destroys the base prescription.
