...
**Design Part:** Day 03  
**Screen / State:** Goal Detail  
**Status:** PRODUCT/UI DECIDED — V1  
**Reference:** `PBOS-Goal-Detail-v1-REFERENCE.png`
...
## Purpose
Goal Detail shows one desired outcome and the system of repeatable behavior/evidence supporting it.

## Locked Information
- Goal identity/outcome.
- Status and meaningful progress.
- Linked Systems.
- Milestones where relevant.
- Linked/next Actions.
- Evidence/trajectory.
- Relevant AI recommendation with explanation and user approval controls where used.

## Locked Behavior
- Goal does not own a duplicate Action list engine; linked Actions come from the shared Action system.
- Systems are the repeatable mechanisms under the Goal.
- A Goal can be paused/completed/inactive without deleting its history.
- Progress should be explainable from milestones/evidence/system activity.
- AI recommendations should identify reason/evidence and support Approve/Modify/Reject rather than automatic mutation.
- Goal Detail is the canonical place to understand the outcome-to-system connection.

## Final-Audit Checks
- Goal → System → Action relationship is visible and real.
- Milestones are not confused with Actions.
- Progress survives status transitions/history.
