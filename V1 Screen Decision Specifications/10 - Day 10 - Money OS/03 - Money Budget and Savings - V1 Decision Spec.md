# PBOS — Money Budget & Savings

**Archive:** V1 Screen Decision Archive  
**Design Part:** Day 10  
**Screen / State:** Money Budget & Savings  
**Status:** PRODUCT/UI DECIDED — V1  
**Reference:** `PBOS-Money-Budget-Savings-v1-REFERENCE.png`
...
### Day 10 Shared Context
- Money OS is lightweight manual personal-finance awareness, not accounting, banking or advanced finance software.
- **Actual transaction ≠ planned expense**.
- **Savings transfer ≠ expense**.
- **PBOS-calculated balance ≠ verified bank balance**.
- Budgets, totals and projections are deterministic; AI may explain patterns and recommend.
- Money remains separate from any general performance score.
- Do not fabricate financial health from missing data.

## Purpose
Budget & Savings defines planned limits/targets and savings awareness without pretending to be a full financial-planning suite.

## Locked Decisions
- Budget is a plan/constraint, not an actual transaction.
- Savings transfer/state is modeled distinctly from expense.
- Remaining budget is based on real configured budget + actual relevant transactions.
- Projections/totals are deterministic.
- AI may suggest saving/spending adjustments but the user decides.
- Avoid fake “financial health score.”
- The original image was generated during the design process; its exact runtime binary was later unavailable due to filename reuse, but the screen decision remains part of V1.

## Final-Audit Checks
- Budget math excludes savings transfers where appropriate.
- Missing budget configuration shows setup/unknown state.
