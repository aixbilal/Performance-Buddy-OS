...
**Design Part:** Day 10  
**Screen / State:** Money Overview  
**Status:** PRODUCT/UI DECIDED — V1  
**Reference:** `PBOS-Money-Overview-v1-REFERENCE.png`
...
## Purpose
Money Overview provides lightweight awareness of recent spending, savings and configured budget position.

## Locked Decisions
- Manual personal finance awareness only; no banking/accounting ambition in V1.
- Show totals/trends only from real recorded transactions/configuration.
- PBOS-calculated balance must not be labeled as verified bank balance.
- Savings transfers are not expenses.
- Planned expenses are not actual transactions.
- Money is excluded from any global performance score.
- AI can explain patterns/recommend but not fabricate or directly mutate financial records.

## Final-Audit Checks
- Missing data is not shown as zero financial behavior.
