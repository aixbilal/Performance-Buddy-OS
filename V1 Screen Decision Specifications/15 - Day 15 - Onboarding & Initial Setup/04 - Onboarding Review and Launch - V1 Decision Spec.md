# PBOS — Onboarding Review & Launch

**Archive:** V1 Screen Decision Archive  
**Design Part:** Day 15  
**Screen / State:** Onboarding Review & Launch  
**Status:** PRODUCT/UI DECIDED — V1  
**Reference:** `PBOS-Onboarding-Review-Launch-v1-REFERENCE.png`
...
## Purpose
Final verification before entering real PBOS. It summarizes; it does not introduce new configuration.

## Locked Sections
- Starting baseline.
- Priorities.
- Connected systems and partial status.
- Planning rules.
- AI/privacy state.
- Data/local storage.
...
## Rules
- “Ready” does not require every domain configured.
- Optional integrations do not block launch.
- Core DB/storage failure may block launch safely.
- No fake historical activity.
- No silent first schedule. First plan should be proposed/reviewed.
- Primary action: **`Launch Performance Buddy OS →`**
- Launch goes directly to real shell/Today without replaying the splash.

## Persistent Routing
- `first_boot_experience_seen = false` → full splash → set true → Welcome.
- seen true + `in_progress` → short splash → Continue Setup.
- seen true + `not_started` → short splash → Welcome/Setup entry.
- seen true + `completed` or `skipped` → short splash → Today.
- Normal short splash target ~0.8–1.2 sec, code-based.
- Reduced Motion uses static/minimal treatment.
- Slow initialization holds final splash state.
- Core failure routes to recovery/error, not fake fresh onboarding.

## Final-Audit Checks
- Launch state persists correctly.
- No splash replay after Review & Launch.
