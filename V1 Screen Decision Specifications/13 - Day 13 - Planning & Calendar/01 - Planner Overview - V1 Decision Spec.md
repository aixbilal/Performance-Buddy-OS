# PBOS — Planner Overview

**Archive:** V1 Screen Decision Archive  
**Design Part:** Day 13  
**Screen / State:** Planner Overview  
**Status:** PRODUCT/UI DECIDED — V1  
**Reference:** `PBOS-Planner-Overview-v1-REFERENCE.png`
...
## Purpose
Planner Overview is the planning workspace for fitting actionable work into real capacity while preserving constraints.

## Locked Decisions
- Planner is not Calendar and not Today.
- Actions are work items; scheduled blocks are time allocations; completion is actual execution.
- Show planning context, capacity, work needing placement, constraints and current plan state.
- Fixed/protected time differs from flexible capacity.
- Empty calendar time is not automatically usable productive capacity.
- Planning respects protected sleep/time and configured buffers.
- AI/planner may generate a proposal; user reviews before applying.
- Existing locked/manual placements survive regeneration.

## Final-Audit Checks
- Planner does not automatically mutate calendar on “Generate.”
- Capacity math is deterministic.
