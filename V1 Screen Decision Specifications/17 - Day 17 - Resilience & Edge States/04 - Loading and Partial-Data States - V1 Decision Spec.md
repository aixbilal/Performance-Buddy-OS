# PBOS — Loading & Partial-Data States

**Archive:** V1 Screen Decision Archive  
**Design Part:** Day 17  
**Screen / State:** Loading & Partial-Data States  
**Status:** PRODUCT/UI DECIDED — V1  
**Reference:** `PBOS-Loading-Partial-State-v1-REFERENCE.png`
...
## Purpose
Show valid information as soon as available while accurately identifying what is still loading, processing, missing, stale or waiting externally.
...
## Startup
Avoid `Splash → giant spinner → another loading page → Today`.
Preferred: Splash → Shell/Today → local components resolve.
Use page-level loading only when nothing meaningful can render.

## Skeletons
Use only when incoming shape is known; restrained pulse, stable layout, Reduced Motion support.
No cyberpunk shimmer.

## Smallest Scope
If only Analytics Summary is loading, Today/Actions/Deadlines remain usable.

## Background Processing
Example Search Index rebuild:
- If real progress known: `642 / 1,104 items`.
- If not measurable: `Rebuilding Search Index…`.
Never fabricate percentages.
Keep PBOS usable where safe.

## Partial Data
Show what is known; identify what is missing; do not derive a value requiring missing input.
Example: known quiz/assignment/mid marks visible, `Projected Grade unavailable — grading setup incomplete`.

## Unknown vs Loading
Loading means a result is expected.
Unknown means no reliable value exists; show `No sleep data recorded`, not endless skeleton.

## Stale Data
Previously valid values can remain visible with `Last updated` + `Refreshing…`.
Stale must be identifiable.

## AI Waiting
Preserve conversation/input; show restrained waiting; navigation remains available where safe; no fake AI progress percentage.

## Saving
Distinguish `Saving…`, `Saved`, `Save Failed`; do not show Saved before authoritative persistence succeeds.

## No Artificial Delay
If local search/calculation is fast, show result immediately.

## Final-Audit Checks
- Loading scope is minimal.
- Real progress only.
- Partial data never becomes guessed value.
