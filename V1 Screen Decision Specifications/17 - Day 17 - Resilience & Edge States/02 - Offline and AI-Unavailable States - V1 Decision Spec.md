# PBOS — Offline & AI-Unavailable States

**Archive:** V1 Screen Decision Archive  
**Design Part:** Day 17  
**Screen / State:** Offline & AI-Unavailable States  
**Status:** PRODUCT/UI DECIDED — V1  
**Reference:** `PBOS-Offline-AI-Unavailable-v1-REFERENCE.png`
...
## Purpose
Graceful degradation when connectivity or AI capability is unavailable.

## Five Conditions
1. Device Offline.
2. AI Disabled by user.
3. AI Not Configured.
4. AI Temporarily Unavailable (provider/network/rate limit).
5. Individual AI Request Failed.

## Device Offline
- Do not replace the whole app with a giant offline page.
- Local PBOS continues wherever possible: Today, Actions, Goals/Systems, Academics/calculations, Knowledge metadata, Development, Fitness, Routines, Reading/Language, Money, Planner, Focus, local Analytics, local Search, Settings, Capture.
- Restrained shell indicator: `Offline · Local features available`.
- Temporary `Back online` may appear when restored.

## AI Disabled
Not an error.
`AI Coach is disabled` → AI Settings / Continue Without AI.

## AI Not Configured
`Set up AI Coach` → Configure AI / Not Now.
No AI setup wall blocks PBOS.

## AI Temporarily Unavailable
`AI is temporarily unavailable` → Retry / Continue Without AI / optional View Status Details.
PBOS data is safe and local features remain available.

## Failed AI Response
- Keep user's message.
- Keep context.
- Inline `Couldn't complete this response`.
- Retry without refreshing/closing the whole workspace.

## Quick Capture Without AI
Offer `Choose Type Manually` or `Save to Capture Inbox`.

## Final-Audit Checks
- External failure affects only the capability that needs it.
- No unnecessary retyping after failure.
