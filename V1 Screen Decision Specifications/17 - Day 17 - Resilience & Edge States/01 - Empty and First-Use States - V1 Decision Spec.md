# PBOS — Empty & First-Use States

**Archive:** V1 Screen Decision Archive  
**Design Part:** Day 17  
**Screen / State:** Empty & First-Use States  
**Status:** PRODUCT/UI DECIDED — V1  
**Reference:** `PBOS-Empty-First-Use-States-v1-REFERENCE.png`
...
## Purpose
Reusable empty-state system that explains why a surface is empty, whether that is expected and the best next action.

## Four Core Empty Types
1. **True Empty** — configured, no records.
2. **Setup Required / First Use** — domain not configured.
3. **Filtered Empty** — data exists, current filters/search have no matches.
4. **Contextual Empty** — parent exists, this subsection has no records.

## Positive Empty
Zero can be a good state:
- Capture Inbox: `You're all caught up`.
- Review Queue: `Nothing needs review`.
- Overdue Actions: `Nothing overdue`.
Keep it quietly positive; no confetti/streak celebration.

## Reusable Anatomy
Icon → Title → one short explanation → primary action → optional secondary action.
No charts, fake metrics, giant illustrations or AI motivational paragraphs.

## Representative Locked Copy/Behavior
- Goals: `No goals yet` → Create Goal.
- Academics not configured → Set Up Academics.
- Today empty → `Your day is open` → Plan Today; secondary Quick Capture.
- Search no result → Clear Search / optional Quick Capture.
- Analytics sparse → `Not enough evidence yet`.
- Capture Inbox → `You're all caught up`.

## Critical Rules
- Loading must not flash as empty.
- Query failure must not be interpreted as zero records.
- Unknown mastery/spending/sleep/coverage is not 0%.
- AI may be a secondary helper, never the only path.

## Final-Audit Checks
- Every empty state matches the actual reason.
- Setup Required is distinguishable from True Empty.
