# PBOS — AI Coach Workspace

**Archive:** V1 Screen Decision Archive  
**Design Part:** Day 12  
**Screen / State:** AI Coach Workspace  
**Status:** PRODUCT/UI DECIDED — V1  
**Reference:** `PBOS-AI-Coach-Workspace-v1-REFERENCE.png`
...
## Purpose
Workspace is the conversational AI surface for asking questions about permitted PBOS context.

## Locked Decisions
- Conversation can answer questions, explain evidence and discuss decisions.
- Conversation itself is not database mutation.
- The user's prompt and relevant context are preserved on a failed request.
- AI must state when evidence/context is missing rather than guessing.
- The workspace can propose structured Actions/schedule changes, but those go through review/validation.
- Context is permission-scoped and minimized.
- Local-first does not imply cloud AI processing is on-device; external processing must be honestly represented.
- AI-disabled core features continue.

## Final-Audit Checks
- Chat messages cannot directly bypass service/domain write rules.
- Failed AI response preserves user input.
