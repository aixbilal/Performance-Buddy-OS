# PBOS — AI Recommendations & Decisions

**Archive:** V1 Screen Decision Archive  
**Design Part:** Day 12  
**Screen / State:** AI Recommendations & Decisions  
**Status:** PRODUCT/UI DECIDED — V1  
**Reference:** `PBOS-AI-Recommendations-Decisions-v1-REFERENCE.png`
...
## Purpose
This screen is the explicit decision surface for reviewing AI proposals before they change PBOS.

## Locked Pattern
**Recommendation → Reason → Evidence → Approve / Modify / Reject**
Confidence may be shown only when actually meaningful and not as fake precision.

## Locked Decisions
- Separate pending recommendations from already-applied changes.
- User can inspect why the recommendation exists.
- Approve routes through deterministic validation and existing domain engine.
- Modify lets the user change the proposal before applying.
- Reject records the decision where useful without punishment.
- No unrestricted AI writes.
- Recommendations may concern Actions, schedule adjustments, Goals/Systems or other supported changes.

## Final-Audit Checks
- Approval is required for meaningful mutation.
- Validation runs after proposal and before persistence.
